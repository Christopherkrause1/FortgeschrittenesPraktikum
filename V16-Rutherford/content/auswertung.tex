...
$\rho_\symup{Au} = \SI{19.32}{\gram\per\centi\meter^3}$ (Quelle) und molaren Masse $M_\symup{Au} = \SI{196.97}{\gram\per\mol}$ (Quelle)
von Gold aus
\begin{equation}
...
differentiellen Wirkungsquerschnitte berechnet werden. Die Ergebnisse sind in Tabelle (...)
...
berechnen. In Tabelle ... sind nun die Wirkunsquerschnitte der $\SI{2}{\micro\meter}$-Folie
...
\begin{table}[H]
  ...
  \begin{tabular}{c c }
    \toprule
    $\frac{\symup{d}\sigma}{\symup{d}\Omega}_\symup{2\mu m} / \cdot 10^{-23}\, m$ & $\frac{\symup{d}\sigma}{\symup{d}\Omega}_\symup{4\mu m}/ \cdot 10^{-23}\, m$ \\
    \midrule
...
Die untersuchten Elemente werden mit ihrer gemessenen Zählrate $I$ und der entsprechenden
Foliendicke in Tabelle ... aufgeführt. Des weiteren sind die Dichte $\rho$ sowie die
...
\begin{table}[H]
  \centering
  \caption{Spezifische Angaben und Zählraten der Untersuchten Folien bei der Untersuchung der Z-Abhängigkeit (Quelle)}
  \label{tab:stoffe}
  \begin{tabular}{c c c c c c c}
    \toprule
     & Z & $\Delta x$/ \mu m & I / $\frac{1}{s}$ & $\rho/ \frac{g}{cm^3}$ & M/$\frac{g}{mol}$ & $\frac{I}{N\cdot \Delta x}$/$\cdot 10^{-23}\frac{m^2}{s}$ \\
    \midrule
